...
import cn.itcasta.lottery0713.util.MemoryManager;
import cn.itcasta.lottery0713.util.SoftReferenceMap;
...
public class UiManager extends Observable {
	...
	private static Map<String, BaseView> VIEWMAP;// key为BaseView子类的简单名称

	static {
		// 判断当前系统内存是否满足应用的需求
		if (MemoryManager.hasAcailMemory()) {
			// 如果满足
			VIEWMAP = new HashMap<String, BaseView>();
		} else {
			// 如果不满足
			VIEWMAP = new SoftReferenceMap<String, BaseView>();
		}
	}
		...
		if (StringUtils.isNotBlank(key)) {
			if (currentView.getClass().getSimpleName().equals(key)) {
				...
				if (targetView == null) {
					...
					clearCache();
					// 提示用户
					PromptManager.showToast(middleContainer.getContext(), "应用在低内存下运行");
				} else {
					middleContainer.removeAllViews();
					middleContainer.addView(targetView.getView());
					currentView = targetView;
